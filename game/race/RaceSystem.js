import { compareRacers, crossedProgress } from './raceMath.js?v=finish-plane';
import { LAPS } from '../config/game-config.js';

export class RaceSystem {
  constructor(racers, track) {
    this.racers = racers; this.track = track; this.state = 'COUNTDOWN'; this.countdown = 3.8; this.goTimer = 0; this.time = 0; this.finishCount = 0; this.resultsTimer = 0; this.lapAnnouncement = null; this.lapAnnouncementSerial = 0;
  ...
  update(dt) {
    ...
    if (this.state === 'COUNTDOWN') { this.countdown -= dt; if (this.countdown <= 0) { this.state = 'RACING'; this.countdown = 0; this.goTimer = .75; this.announceLap(1); } return; }
  ...
  updateRacer(racer, dt) {
    ...
    const previousProgress = racer.previousProgress;
    const delta = kart.progress - previousProgress; const wrapped = delta > .5 ? delta - 1 : delta < -.5 ? delta + 1 : delta;
    ...
    if (crossedProgress(previousProgress, kart.progress, target) && Math.abs(this.track.query(kart.position, kart.progress).lateral) < this.track.width * .62) {
      if (racer.nextGate === 0) {
        ...
        racer.nextGate = 1;
        if (racer.player && racer.lap < LAPS) this.announceLap(racer.lap + 1);
  ...
  announceLap(lap) {
    this.lapAnnouncement = { lap, total: LAPS, serial: ++this.lapAnnouncementSerial };
  }
